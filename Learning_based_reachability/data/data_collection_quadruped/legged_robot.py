...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: Cfg, sim_params, physics_engine, sim_device, headless, eval_cfg=None,
                 initial_dynamics_dict=None, init_scheme="verification"):
        ...
        if not self.headless:
            # self.cfg.viewer.pos=[16.0, 6.0, 6.0]
            # self.cfg.viewer.lookat=[16.0, 6.0, 3]
            self.cfg.viewer.pos=[6.0, 6.0, 6.0]
            self.cfg.viewer.lookat=[6.0, 6.0001, 3]
            print(self.cfg.viewer.pos,self.cfg.viewer.lookat)
            self.set_camera(self.cfg.viewer.pos, self.cfg.viewer.lookat)
            # self.set_camera([6, 6, 5], [6, 6, 0.1])
        self._init_buffers()
        ...
        self.y_init=None
        if self.init_scheme=="safety_filter":
            self.y_init=-3.0
            # self.gym.set_light_parameters(
            #     self.sim, 0, gymapi.Vec3(0.99, 0.99, 0.99), gymapi.Vec3(0.6, 0.6, 0.6), gymapi.Vec3(0.1, 0.1, 0.01)
            # )
            self.gym.set_light_parameters(self.sim, 0, gymapi.Vec3(0.8, 0.8, 0.8), gymapi.Vec3(0.6, 0.6, 0.6), gymapi.Vec3(0, 0, -3))

    def load_cfg(self, cfg: Cfg, headless, eval_cfg=None, deploy=True, prone=False, num_envs=None):
        self.cfg = cfg
        self.eval_cfg = eval_cfg
        ...
        if deploy:  # turn off randomization, fix terrain
            ...
            self.cfg.terrain.border_size = 50
            self.cfg.noise.add_noise = False
            self.cfg.domain_rand.push_robots = False
            self.cfg.domain_rand.randomize_friction = False
            self.cfg.env.episode_length_s = 2000000
    ...
    def record_state(self):
        """ Apply actions, simulate, and then undo the changes

        Args:
            actions (torch.Tensor): Tensor of shape (num_envs, num_actions_per_env)
        """
        # store previous states
        self.root_states_pre=to_torch(self.root_states.clone())
        self.dof_state_pre=to_torch(self.dof_state.clone())

        self.org_obs_buf=self.obs_buf.clone()
        self.org_privileged_obs_buf=self.privileged_obs_buf.clone()

    def undo(self):
        root_states_post=self.root_states.clone()
        dof_pos_post=self.dof_pos.clone()
        dof_vel_post=self.dof_vel.clone()
        base_quat_ = self.root_states[:, 3:7].clone()
        base_lin_vel_ = quat_rotate_inverse(base_quat_, root_states_post[:, 7:10])
        base_ang_vel_ = quat_rotate_inverse(base_quat_, root_states_post[:, 10:13])

        gravity_projected_=quat_rotate_inverse(self.base_quat, self.gravity_vec)

        contact_indicator_=torch.abs(torch.sign(self.net_contact_forces.reshape(self.num_envs,13,3)[:,3:13:3,-1]))
        
        # do a proper reset
        self.reset()
        self._call_train_eval(self._shift_robot, torch.arange(self.num_envs, device=self.device))
        self._call_train_eval(self._reset_robots_dof, torch.arange(self.num_envs, device=self.device))
        self.obs_buf=self.org_obs_buf.clone()
        self.privileged_obs_buf=self.org_privileged_obs_buf.clone()
        
        
        return root_states_post[...,:3].cpu(), base_quat_.cpu(), base_lin_vel_.cpu(),\
              base_ang_vel_.cpu(), contact_indicator_.cpu(), dof_pos_post.cpu(),dof_vel_post.cpu(), gravity_projected_.cpu()

    
    def _shift_robot(self, env_ids, cfg):
        self.root_states[...] = self.root_states_pre
        self.gym.set_actor_root_state_tensor(self.sim, gymtorch.unwrap_tensor(self.root_states))

    def _reset_robots_dof(self, env_ids, cfg):
        self.dof_state[...] = self.dof_state_pre
    ...
    def post_physics_step(self):
        ...
        # TODO: disable it for verification!
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        if self.custom_origins:
            if self.init_scheme=="verification":
                self.env_origins[env_ids, :2]= 6.0
                ...
                self.root_states[env_ids, :2] +=torch.zeros_like(self.root_states[env_ids, :2]).uniform_(-2.0,2.0)-10.0
                                                    # torch_rand_float(-2,
                                                    #             2, (len(env_ids), 2),
                                                    #             device=self.device) # x y
                # yaw
                yaws=torch.zeros(len(env_ids),1).uniform_(-torch.pi,torch.pi).cuda()
                self.root_states[env_ids, 5:6]=torch.sin(yaws * 0.5)
                self.root_states[env_ids, 6:7]=torch.cos(yaws * 0.5)

                vx_b=torch_rand_float(0.0,3.0, (len(env_ids), 1),device=self.device)
                vy_b=torch_rand_float(-0.5,0.5, (len(env_ids), 1),device=self.device)

                # convert body frame v to world frame
                self.root_states[env_ids, 7:8] = vx_b* torch.cos(yaws) - vy_b *torch.sin(yaws) # vx
                self.root_states[env_ids, 8:9] = vx_b* torch.sin(yaws) + vy_b *torch.cos(yaws) # vx

                self.root_states[env_ids, 12] = torch_rand_float(-2.0,
                                                                2.0, (len(env_ids),2),
                                                                device=self.device)[:,0] # omega
            elif self.init_scheme=="safety_filter":
                self.env_origins[env_ids, :2]= 6.0
                self.root_states[env_ids] = self.base_init_state.clone()
                self.root_states[env_ids, :3] += self.env_origins[env_ids]
                # self.root_states[env_ids, :2] += torch_rand_float(cfg.terrain.x_init_range,
                ...
                # self.root_states[env_ids, :2] += torch_rand_float(-3,
                #                                                 3, (len(env_ids), 2),
                #                                                 device=self.device) # x y

                # xys = torch.cartesian_prod(torch.linspace(-3,3,50), torch.linspace(-3,3,50)).cuda()
                # self.root_states[env_ids, :2] += xys
                self.root_states[env_ids, 0]+=0.428
                self.root_states[env_ids, 1]+=-2.573

                # self.root_states[env_ids, 0]+=torch.linspace(-3.0,3.0,101).cuda()
                # self.root_states[env_ids, 1]+=self.y_init
                
                # yaw
                yaws=torch.ones(len(env_ids),1).cuda()*1.5708
                self.root_states[env_ids, 5:6]=torch.sin(yaws * 0.5)
                self.root_states[env_ids, 6:7]=torch.cos(yaws * 0.5)

                vx_b=torch_rand_float(2.99,3.0, (len(env_ids), 1),device=self.device)
                vy_b=torch_rand_float(-0.001,0.001, (len(env_ids), 1),device=self.device)

                # convert body frame v to world frame
                self.root_states[env_ids, 7:8] = vx_b* torch.cos(yaws) - vy_b *torch.sin(yaws) # vx
                self.root_states[env_ids, 8:9] = vx_b* torch.sin(yaws) + vy_b *torch.cos(yaws) # vx

                self.root_states[env_ids, 12] = 0.0 # omega
            else:
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        if cfg.terrain.teleport_robots:
            ...
            low_x_ids = env_ids[self.root_states[env_ids, 0] < -20]
            self.root_states[low_x_ids, 0] += 75

            high_x_ids = env_ids[
                self.root_states[env_ids, 0] > 60]
            self.root_states[high_x_ids, 0] -= 75
            low_y_ids = env_ids[self.root_states[env_ids, 1] < -25]
            self.root_states[low_y_ids, 1] += 80

            high_y_ids = env_ids[
                self.root_states[env_ids, 1] > 60]
            self.root_states[high_y_ids, 1] -= 80
