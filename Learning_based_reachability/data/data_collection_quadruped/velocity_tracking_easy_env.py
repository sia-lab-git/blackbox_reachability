...
class VelocityTrackingEasyEnv(LeggedRobot):
    def __init__(self, sim_device, headless, num_envs=None, prone=False, deploy=True, init_scheme="verification",
        ...
        if deploy:  # turn off randomization, fix terrain
            ...
            cfg.domain_rand.push_robots = False
            cfg.domain_rand.randomize_friction = False
            cfg.env.episode_length_s = 2000000

        sim_params = gymapi.SimParams()
        gymutil.parse_sim_config(vars(cfg.sim), sim_params)
        super().__init__(cfg, sim_params, physics_engine, sim_device, headless, eval_cfg, initial_dynamics_dict, init_scheme=init_scheme)
